...
public class PlayerKillsMobListener implements Listener {
	...
	public void OnKill(final EntityDeathEvent event) {
		...
		if (!plugin.getDependencyManager().getDependency(dependency.STATS).isAvailable()) return;
		...
		if (e.getLastDamageCause() instanceof EntityDamageByEntityEvent) {
			...
			if (nEvent.getDamager() instanceof Player) {
				...
				StatsAPIHandler handler = (StatsAPIHandler) plugin.getDependencyManager().getDependency(dependency.STATS);

				Stat mobkilled = handler.getAPI().getStatManager().getStat(MobKilledStat.statName);
				
				StatsHolder holder = handler.getAPI().getPlayer(nEvent.getDamager().getUniqueId());

 ...
 * When the last known group is not equal to the current group of a player, all progress should be reset as a player is not longer in the same group.
 * 
 * RequirementHandler uses a file (/playerdata/playerdata.yml) which keeps tracks of these things.
 * @author Staartvin
 *
 */
public class RequirementHandler {
	...
	public void createNewFile() {
		...
		plugin.getLogger().info("New playerdata.yml file loaded. AR can keep track of player progress now!");
	}
	...
	public void loadConfig() {
		...
		config.options().header("This file saves all progress of players." +
	...
	public List<Integer> getProgress(String playerName) {
		return (List<Integer>) config.getList(playerName + ".progress", new ArrayList<Integer>());
	}
	
	public void addPlayerProgress(String playerName, int reqID) {
		List<Integer> progress = getProgress(playerName);
		
		if (hasCompletedRequirement(reqID, playerName)) return;
